...
public class HuffmanAlgorithm {
    public static void main(String[] args) {
        ...
        TreeMap<Character, Integer> frequency = countFrequency(text);
        ...
        for (int i = 0; i < text.length(); i++) {
            encoder.append(codes.get(text.charAt(i)));
        }
        System.out.println("Размер исходной строки: " + text.getBytes().length * 8 + " бит");
        System.out.println("Размер сжатой строки: " + encoder.length()  + " бит");
        System.out.println("Биты сжатой строки: " + encoder);

        // декодируем сжатую информацию обратно
        String decoded = huffmanDecode(encoder.toString(), tree);
